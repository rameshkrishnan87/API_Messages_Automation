...
Feature: To create a new booking in restful-booker
  ...
  Scenario Outline: To create new message using cucumber Data Table
   ...
   # And user validates the response with JSON schema "createMessageSchema.json"
    And user validates the response with Open API

    Examples:
      |name   | email          | phone       | subject         | description                        |
      | John  | john@test.com  | 32465806421 | Create Message 3| Create Message 1 -Description Demo |
      | Jack  | jack@test.com  | 32465280160 | Create Message 4| Create Message 1 -Description Demo |